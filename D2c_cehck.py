...
reluctant_features = ['id', 'signup_date', 'buy']
...
X_train, X_val, Y_train, Y_val = train_test_split(X_over, Y_over, test_size = 0.2, random_state = 45)
'''
# Handling imbalance dataset
print(Counter(Y_train))
smk = SMOTE(random_state = 45, sampling_strategy=0.1)
X_over, Y_over = smk.fit_resample(X_train, Y_train)
...
optimizer = optim.Adam(model.parameters(), lr= 0.0001)#LEARNING_RATE)


th = 0.55# accuracy prediction
def binary_acc(y_pred, y_test):
    y_pred = torch.sigmoid(y_pred)
      #print(y[y>0.5])
      #print((y>0.5).float())
    y_pred_tag = (y_pred>th).float()
...
for e in range(1, 1+1):
  ...
  with torch.no_grad():
    for X_batch_val, y_batch_val in val_loader:
        ...
        y_pred_tag = (y_test_pred>th).float()
        #y_pred_tag = torch.round(torch.sigmoid(y_test_pred))
        acc = f1_score(y_pred_tag, y_batch_val.unsqueeze(1))
  ...
  if not os.path.exists(directory):
    os.mkdir(directory)
  torch.save(model.state_dict(), directory + "Epoch_"+str(e)+"_loss_"+str(acc))
      

  print(f'Epoch {e+0:03}: | Train_Loss: {epoch_loss/len(train_loader):.5f} | Val_Loss: {loss.item():.5f}| F1_Score: {acc:.3f}')
...
print(confusion_matrix(y_pred_list[0], Y_val))
print(Counter(Y_val))
print(Counter(y_pred_list[0]))
from sklearn.metrics import roc_auc_score
print(roc_auc_score(Y_val, y_pred_list[0]))
from sklearn.metrics import precision_score
precision = precision_score(Y_val, y_pred_list[0], average='binary')
print('Precision: %.3f' % precision)

from sklearn.metrics import recall_score
recall = recall_score(Y_val, y_pred_list[0], average='binary')
print('Recall: %.3f' % recall)

X_test = pd.read_csv('DATA/test_Wf7sxXF.csv', sep = ',', parse_dates = ['created_at'])
X_test.fillna({'products_purchased':0}, inplace = True)
X_test['created_at'] = X_test['created_at'].astype(int)
